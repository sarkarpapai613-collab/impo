...
const SECRET = "031225";      // password (case-insensitive)

// 2) Names
const GIRLFRIEND_NAME  = "Baby";
const GIRLFRIEND_NAME2 = "Yashasvi";
const YOUR_NAME        = "Jericho";

// 3) Rose Day date (Feb 7) - used only for countdown
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // If this script is included on a page that doesn't have rose layout, just exit safely
  if (!lock || !app) return;
...
No distance, no day, no person changes that.`;
  ...
  // small animation (inject once)
  if (!document.getElementById("__fadeInUpStyle")){
    const style = document.createElement("style");
    style.id = "__fadeInUpStyle";
    style.textContent = `
      @keyframes fadeInUp{
...
And I’m not letting you go. 😚💗`;
  ...
  surpriseBtn?.addEventListener("click", () => {
    if (modalMsg) modalMsg.textContent = surpriseText;
  ...
  noBtn?.addEventListener("pointerdown", (e) => {
    e.preventDefault(); e.stopPropagation(); escapeNoButton();
  });
  ...
  function escapeNoButton(){
    ...
    noBtn.style.left = `${clamp(x, -180, 180)}px`;
    noBtn.style.top  = `${clamp(y, -80, 80)}px`;
    ...
    setTimeout(() => { noBtn.style.pointerEvents = "auto"; }, 180);
  }
  ...
  musicBtn?.addEventListener("click", async () => {
    ...
    try{
      ...
      alert("Autoplay blocked 😅 Tap again or add a valid dhun.mp3 file.");
    }
  });
